...
Pass `k_shortest_paths()` a data frame (`graph_df`) with rows representing the edges of a graph, as well as a `start_vertex` and `end_vertex`, referencing nodes in `graph_df`. The function will return a list containing up to `k` (by default 1) vectors representing paths through the graph:
...
end nodes, respectively. These maay be either integers or character vectors. If
there is a column named "weights", it is used for edge weights.  You can change
these defaults by passing column numbers or names to the `from`, `to`, and
`weights` arguments.
...
library(igraphdata)
data("USairports")
paths <- k_shortest_paths(USairports, "BGR", "OMA", k = 20, weights = "Distance")
paths[1:5]
```
...
bench::mark(
  ...
  k_shortest_paths(network, start_vertex = 1, end_vertex = 10, k = 1),
  check = FALSE
)
```
...
bench::mark(
  ...
  k_shortest_paths(network, start_vertex = 1, end_vertex = 5, k = 1),
  check = FALSE
)
```
