...
Pass `k_shortest_paths()` a data frame (`graph_df`) with rows representing the edges of a graph. The first and second columns represent the start and end nodes, and the optional third column contains a numeric vector of edge weights. You must also provide a `start_vertex` and `end_vertex`, which each specify a node as it is referenced in `graph_df`. Nodes can be represented as integer or character vectors.
